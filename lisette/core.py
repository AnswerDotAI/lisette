...
# %% auto #0
__all__ = ['haik45', 'sonn45', 'sonn', 'sonn46', 'opus46', 'opus', 'tool_dtls_tag', 're_tools', 'token_dtls_tag', 're_token',
           'stream_chunk_builder', 'effort', 'tc_res_sysp', 'status_re', 'patch_litellm', 'remove_cache_ckpts',
...
def _think_kw(model, think):
    ...
    if not think: return {}
    e = effort.get(think)
    if 'opus-4-7' in model: return dict(thinking={"type":"adaptive", "display": "summarized"}, output_config={"effort":e})
...
def _prep_call(self:Chat, prefill, search, max_tokens, kwargs, stream=False, think=None):
    ...
    if 'web_search_options' in kwargs and self.tool_schemas and 'gemini' in self.model: kwargs['include_server_side_tool_invocations'] = True
