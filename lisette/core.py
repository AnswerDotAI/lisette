...
# %% auto 0
__all__ = ['sonn45', 'detls_tag', 'effort', 'patch_litellm', 'mk_msg', 'fmt2hist', 'mk_msgs', 'stream_with_complete',
...
def _extract_tool(text:str)->tuple[dict,dict]:
    ...
    if m:=re.search(r'<details.*?```json\n(.*?)\n```.*?</details>', text, re.DOTALL): 
...
def fmt2hist(fmt_outp:str)->list:
    "Transform a formatted output into a LiteLLM compatible history"
    lm,hist = Message(),[]
    for p in re.split(fr"(\n\n{detls_tag}\n\n```json\n.*?\n```\n\n</details>\n\n)", fmt_outp, flags=re.DOTALL):
        if tcr:=_extract_tool(p):
            if not hist: hist.append(lm) # case when LLM calls a tool without talking
            lm.tool_calls = lm.tool_calls+[tcr[0]] if lm.tool_calls else [tcr[0]] 
            hist.append(tcr[1])
        elif p: hist.append(lm:=Message(p))
    return hist
