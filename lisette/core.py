...
# %% auto #0
__all__ = ['haik45', 'sonn45', 'sonn', 'sonn46', 'opus46', 'opus', 'gpt54', 'gpt54m', 'tool_info', 'usage_info',
           'stream_chunk_builder', 'codex54m', 'codex54', 'codex55', 'codex53spark', 'effort', 'tc_res_sysp', 'kimi',
           'qwen3p6p', 'qwen_info', 'status_re', 'dsf', 'dsp', 'v4_flash_info', 'v4_pro_info', 'patch_litellm',
           'remove_cache_ckpts', 'contents', 'stop_reason', 'mk_msg', 'parse_tools', 'strip_tools', 'conv_tools',
...
# %% ../nbs/00_core.ipynb #75dcaa40
register_model({"claude-opus-4-7": dict(get_model_info("claude-opus-4-6"))})
...
gpt54 = "openai/responses/gpt-5.4"
gpt54m = "openai/responses/gpt-5.4-mini"
...
# Frozen legacy envelope recognition, used only by `conv_tools`: the exact
# patterns lisette shipped for `<details markdown='1'>` envelopes plus the
# never-released `::: {.details}` spelling.
...
_lg_token = re.compile(
    fr"^(?:{re.escape(_lg_token_tag)}<summary>(?P<tsumm1>.*?)</summary>\n*\n*`(?P<trepr1>.*?)`\n*\n*</details>"
...
def _extract_tool(d:dict)->tuple:
    "Build (tool_call, tool_result_msg) from a parsed `{.tool}` block"
    if not d or d.get('server') or d.get('id') is None: return
    tc = ChatCompletionMessageToolCall(Function(dumps(d.get('args') or {}), d['name']), d['id'])
    tr = {'role': 'tool','tool_call_id': d['id'],'name': d['name'], 'content': d.get('result')}
    return tc,tr
...
def fmt2hist(outp:str)->list:
    ...
    if tool_info not in outp: return [outp]
    for is_last,(txt,d) in loop_last(parse_tools(outp.strip())):
        if is_last and not (txt or '').strip() and not d: continue
        txt = txt.strip() if d or txt.strip() else '.'
        hist.append(lm:=Message(txt))
        if d:
            if tcr := _extract_tool(d):
...
def _setup_gptinfo(mnf, mnt):
    info = dict(get_model_info(mnf))
    info['key'] = mnt
    info['supports_web_search'] = True
    info['supported_openai_params'] = list(info.get('supported_openai_params') or []) + ['web_search_options']
    register_model({mnt: info})
    info.pop('mode', None)
    litellm.model_cost[mnt] = info

_setup_gptinfo("openai/gpt-5.4", gpt54)
_setup_gptinfo("openai/gpt-5.4-mini", gpt54m)
...
def _tc_summary(tc, tr=None):
    ...
    txt = f"{tc.function.name}({params}){res}"
...
def mk_tr_details(tr, tc, mx=2000):
    ...
    args = {k:_trunc_str(v, mx=mx*5) if isinstance(v, str) else v for k,v in json.loads(tc.function.arguments).items()}
    res = dict(id=tr['tool_call_id'], name=tc.function.name, args=args, result=_trunc_content(tr.get('content'), mx=mx))
