...
# %% auto 0
__all__ = ['sonn45', 'effort', 'detls_tag', 'patch_litellm', 'mk_msg', 'stream_with_complete', 'lite_mk_func', 'cite_footnote',
           'cite_footnotes', 'Chat', 'random_tool_id', 'mk_tc', 'mk_tc_req', 'mk_tc_result', 'mk_tc_results', 'mk_msgs',
           'astream_with_complete', 'AsyncChat', 'mk_tr_details', 'AsyncStreamFormatter']
...
def _build_tool_hist(msg):
    ...
    hist = []
    parts = re.split(rf'({detls_tag}.*?</details>)', msg, flags=re.DOTALL)
    for islast, (i,o) in loop_last(enumerate(parts)):
        if detls_tag not in o:
...
def mk_msgs(msgs,                   # List of messages (each: str, bytes, list, or dict w 'role' and 'content' fields)
    ...
    res,role = [],'user'
    msgs = L(msgs).map(lambda m: _build_tool_hist(m) if detls_tag in m else [m]).concat()
    for m in msgs:
...
class AsyncStreamFormatter:
    def __init__(self, include_usage=False):
        self.tcs,self.include_usage,self.think = {},include_usage,False
