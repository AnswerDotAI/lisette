...
class LisetteUsageLogger(CustomLogger):
    def __init__(self, db_path): 
        ...
        if 'usage' not in self.db.t: self.db.create(Usage)
        self.db.usages = self.db.t.usage
        self.db.usages.cls = Usage
    ...
    def _log_usage(self, response_obj, response_cost, start_time, end_time):
        ...
        self.db.usages.insert(Usage(timestamp=time.time(), model=response_obj.model, user_id=self.user_id_fn(), prompt_tokens=usage.prompt_tokens, completion_tokens=usage.completion_tokens,
